...
module.exports = {
    params(ctx) {
        const links = Object.values(OneDrive.oauth2s).map(
            ({oauth_url, client_id, scope, redirect_uri}) => `${oauth_url}authorize?scope=${encodeURIComponent(scope)}&response_type=code&client_id=${client_id}&redirect_uri=${encodeURIComponent(redirect_uri)}`
        );
        const desc = `<div><h3>快速获取code</h3><span><a href="${links[0]}" target="_blank">国际版</a></span><span style="margin-left: 10px"><a href="${links[1]}" target="_blank">世纪互联</a></span></div>`;
        return [
...
class OneDrive {
    ...
    async uploadSession(path, name, size) {
        return this.service.post('items/' + (await this.getIdByPath(path)) + '/createUploadSession', {
            item: {
                '@microsoft.graph.conflictBehavior': 'fail',
                fileSize: size,
                name,
