			...
			(res) => {
				...
				if (res.statusCode === 200) {
					...
					res.on('end', () => {
						...
						console.log("get data from graph: success");
...
exports.func = async (spConfig, cache, request) => {
	...
	if (img_path && p2.startsWith(img_path)) {
		if (p2 === img_path) {//图床上传
			if (request.httpMethod === 'POST' && req_body_json['guest_upload_filebase64']) {
				...
				return Msg_301(request.url_ph01 + img_path + upload_filename + '?preview&refresh');
			} else {
	...
	if (!request.queryString['refresh']) {
...
//exports.func(spConfig_example, {}, { url_p2: '/', queryString: {} });
