...
async function downloadForProxy({url, headers, body, method}, reqHeaders, resHeaders) {
    ...
    if (url.startsWith('file://')) {
        ...
        if (reqHttpRange) {
            ...
            const end = Number(m.groups.start) || size - 1;
...
const server = http.createServer((req, res) => {
    ...
    new Promise((resolve) => {
        ...
        res.end(JSON.stringify({error: err.type || 'UnknownError', data: err.data || {}, msg: err.message}));
    });
}).listen(PORT);

if (server.address()) {
    logger.log(`Running on ${process.platform}, port:${server.address().port}`);
    Object.values(require('os').networkInterfaces()).map(e => e[0].family === 'IPv6' ? e[1].address : e[0].address).filter(e => e).sort().forEach(e => logger.log(`http://${e}:${PORT}`));
}
