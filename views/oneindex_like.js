...
function r200_list(data, readMe, script, splitPath, G_CONFIG) {
    ...
    data.content.forEach(e => {
        if (e.nodeType === 1) {//文件夹
            html += `<li class="mdui-list-item dir mdui-ripple realFile"><a href="${p_h01}${e.url_p2}"><div class="mdui-col-xs-12 mdui-col-sm-7 mdui-text-truncate"><i class="mdui-icon material-icons">folder_open</i><span>${e.name}</span></div><div class="mdui-col-sm-3 mdui-text-right">${e.modified}</div><div class="mdui-col-sm-2 mdui-text-right">${e.size}</div></a></li>`;
            ...
            html += `<li class="mdui-list-item drive mdui-ripple realFile"><a href="${p_h01}${e.url_p2}"><div class="mdui-col-xs-12 mdui-col-sm-7 mdui-text-truncate"><i class="mdui-icon material-icons">folder_open</i><span>${e.name}</span></div><div class="mdui-col-sm-3 mdui-text-right">${e.modified}</div><div class="mdui-col-sm-2 mdui-text-right">${e.size}</div></a></li>`;
        }
    });
...
function get_html_readMe(readMe) {
    ...
    html += `<div id="readMe"></div><textarea id="readme-raw-text" style="display:none;">${readText}</textarea></div>`;
