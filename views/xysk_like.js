...
function get_html_head(p_h0, p_12, G_CONFIG) {
    ...
    p_12.split('/').forEach((e, i, t) => {
        if (e) {
            ...
            html += `<span class="nav-path"><a href="${tmpIncPath}" style="text-decoration:none;">${e}</a></span>`;
...
function get_html_readMe(readMe) {
    ...
    if (readMe.type === 1) {
        ...
        readText += readMe.txt;
        html += `<script>let xhr=new XMLHttpRequest();xhr.open("GET",'${readMe.txt}');xhr.send();xhr.onload=function(){if(xhr.status==200){ document.getElementById('readMe').innerHTML =marked(xhr.responseText)}else{alert("something unexpected wrong");}}</script>`;
    } else if (readMe.type === 2) {
        readText = readMe.txt;
    }
    html += `<div id="readMe" class="markdown-body"></div><textarea id="readme-raw-text" style="display:none;">${readText}</textarea></div></div>`;
